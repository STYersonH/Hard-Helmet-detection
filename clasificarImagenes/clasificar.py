...
def clasificar_imagen(path, i):
    ...
    filename = f"hard_hat_workers{400+i}_png.rf.{unique_id}.jpg"

    # Open the file for writing
    f = open(f'labels/hard_hat_workers{400+i}_png.rf.{unique_id}.txt', 'a')

    def click_event(event, x, y, flags, params):
        nonlocal first_point, second_point, drawing, rectangle_complete, boxes, padded_img, f
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if not drawing:
                ...
                with open(f'labels/hard_hat_workers{400+i}_png.rf.{unique_id}.txt', 'a') as f:
    ...
    filename = f"hard_hat_workers{400+i}.rf.{uuid.uuid4().hex}.jpg"
